...
def _parallel_cov_initializer(gpr):
    global _PARALLEL_COV_GPR
    _PARALLEL_COV_GPR = gpr


def _parallel_cov_worker(task):
    kernel_index, ddir, mol_ids, get_orb_deriv, save_refs = task
    gpr = _PARALLEL_COV_GPR
    kernel = gpr.kernels[kernel_index]
    gpr._compute_mol_covs(
    ...
    result = {
        ...
        "ks_baseline": {
            ...
            for mol_id in mol_ids
            if mol_id in gpr.ks_baseline_dict
        },
        "vdw_terms": {
            mol_id: gpr.vdw_terms_dict[mol_id]
            for mol_id in mol_ids
            if mol_id in gpr.vdw_terms_dict
        },
    }
    return result


def _chunks(seq, chunksize):
    for i in range(0, len(seq), chunksize):
        yield seq[i : i + chunksize]


def _orb_deriv_explicitly_requested(get_orb_deriv):
    if isinstance(get_orb_deriv, (list, tuple)):
        return any(item is True for item in get_orb_deriv)
    return get_orb_deriv is True
...
def _sanitize_vdw_value(v):
    """
    Convert common HDF5-loaded / numpy scalar types into plain python types so
    the object can be cleanly serialized (joblib + YAML) and compared.
    """
    try:
        if isinstance(v, np.generic):
            return _sanitize_vdw_value(v.item())
    except Exception:
        pass
...
class MOLGP:
    ...
    def __init__(
        ...
        # Optional vdW correction terms loaded from REF HDF5 files generated by orchard.
        # Expected schema (in train_data):
        #   vdw: { "units": "Ha", "terms": {<vdw_term>: {"energy_ha": float, ...}, ... } }
        self.vdw_terms_dict = {}
        # Records which vdW term (if any) was used at fit-time to modify y.
        # This is part of the trained model contract and must survive joblib -> mapped YAML.
        self.vdw_fit_term = None
        self.vdw_fit_info = None
        self.vdw_eval_mode = None
        self.exx_ref_dict = {}
        self.dexx_ref_dict = {}
        ...
        self.corr_dcov_dict = {}
        # Reaction bookkeeping (enables optional fit-time vdW subtraction)
        self.rxn_mode_list = []
        self.rxn_structs_list = []
        self.rxn_counts_list = []

    def __setstate__(self, state):
        self.__dict__.update(state)
        if not hasattr(self, "vdw_terms_dict"):
            self.vdw_terms_dict = {}
        if not hasattr(self, "vdw_fit_term"):
            self.vdw_fit_term = None
        if not hasattr(self, "vdw_fit_info"):
            self.vdw_fit_info = None
        if not hasattr(self, "vdw_eval_mode"):
            self.vdw_eval_mode = None
        if not hasattr(self, "rxn_mode_list"):
            self.rxn_mode_list = []
        if not hasattr(self, "rxn_structs_list"):
            self.rxn_structs_list = []
        if not hasattr(self, "rxn_counts_list"):
            self.rxn_counts_list = []

    def _infer_and_validate_vdw_fit_info(self, vdw_term):
        """
        Infer (units, kind, params) for vdw_term and validate consistency across
        all XC-mode reactions. Refuses to silently mix inconsistent vdW metadata.
        """
        struct_ids = set()
        for mode, structs in zip(self.rxn_mode_list, self.rxn_structs_list):
            if mode != 2:
                continue
            for sysid in structs:
                struct_id = sysid[0] if isinstance(sysid, tuple) else sysid
                if isinstance(struct_id, str) and struct_id.startswith("_UEG_"):
                    continue
                struct_ids.add(struct_id)

        exemplar = None
        exemplar_info = None
        for struct_id in sorted(struct_ids):
            vdw = self.vdw_terms_dict.get(struct_id)
            if vdw is None or not isinstance(vdw, dict):
                raise ValueError(
                    f"vdW term '{vdw_term}' requested but vdW data missing for system '{struct_id}'. "
                    "Refuse to mix vdW/non-vdW datasets."
                )
            units = _sanitize_vdw_value(vdw.get("units"))
            terms = vdw.get("terms")
            if terms is None or not isinstance(terms, dict):
                raise ValueError(
                    f"vdW term '{vdw_term}' requested but vdW terms missing for system '{struct_id}'."
                )
            tdata = terms.get(vdw_term)
            if tdata is None or not isinstance(tdata, dict):
                raise ValueError(
                    f"vdW term '{vdw_term}' missing for system '{struct_id}'. "
                    "Refuse to mix vdW/non-vdW datasets."
                )
            kind = _sanitize_vdw_value(tdata.get("kind"))
            params = _sanitize_vdw_value(tdata.get("params")) or {}
            info = {"units": units, "kind": kind, "params": params}

            if exemplar is None:
                exemplar = struct_id
                exemplar_info = info
            elif info != exemplar_info:
                raise ValueError(
                    "Inconsistent vdW metadata for term "
                    f"'{vdw_term}'. Example '{exemplar}' has {exemplar_info}, "
                    f"but '{struct_id}' has {info}. Refuse to mix."
                )

        if exemplar_info is None:
            return None
        return {"term": vdw_term, **exemplar_info}
    ...
    def map(self, mapping_plans):
        ...
        mapped.vdw_fit_term = getattr(self, "vdw_fit_term", None)
        mapped.vdw_fit_info = getattr(self, "vdw_fit_info", None)
        mapped.vdw_eval_mode = getattr(self, "vdw_eval_mode", None)
        return mapped
    ...
    def _get_vdw_energy_ha(self, struct_id, vdw_term):
        if isinstance(struct_id, str) and struct_id.startswith("_UEG_"):
            return 0.0
        vdw = self.vdw_terms_dict.get(struct_id)
        if vdw is None or not isinstance(vdw, dict):
        ...
        if tdata is None or not isinstance(tdata, dict):
            return None
        e = tdata.get("energy_ha")
        if e is None:
            return None
        return float(e)

    def _get_vdw_rxn_corrections(self, vdw_term):
        corr = np.zeros(len(self.rxn_ref_list), dtype=np.float64)
        for i, mode in enumerate(self.rxn_mode_list):
            if mode != 2:
                continue
            structs = self.rxn_structs_list[i]
            counts = self.rxn_counts_list[i]
            for sysid, count in zip(structs, counts):
                struct_id = sysid[0] if isinstance(sysid, tuple) else sysid
                e_vdw = self._get_vdw_energy_ha(struct_id, vdw_term)
                if e_vdw is None:
                    raise ValueError(
                        f"vdW term '{vdw_term}' missing for system '{struct_id}'. "
                        "Refuse to mix vdW/non-vdW datasets."
                    )
                corr[i] += float(count) * e_vdw
        return corr

    def fit(self, x=None, sigma_min=0.25, vdw_term=None):
        Kimn_list = []
        Knmimn = 0
        ...
        y = np.array(self.rxn_ref_list, dtype=np.float64)
        if vdw_term is not None:
            if not (
                hasattr(self, "rxn_mode_list")
                and hasattr(self, "rxn_structs_list")
                and hasattr(self, "rxn_counts_list")
            ):
                raise RuntimeError(
                    "vdW subtraction requested but reaction bookkeeping is missing. "
                    "Recreate the model/reactions with the current code before fitting."
                )
            if len(self.rxn_mode_list) != len(self.rxn_ref_list):
                raise RuntimeError(
                    "vdW subtraction requested but reaction bookkeeping length mismatches. "
                    "Recreate the model/reactions with the current code before fitting."
                )
            # Subtract the vdW contribution to the reaction energy:
            #   E_vdw(rxn) = sum_i count_i * E_vdw(struct_i)
            # with the same sign convention as other baseline subtractions.
            y = y - self._get_vdw_rxn_corrections(vdw_term)
            self.vdw_fit_term = vdw_term
            self.vdw_fit_info = self._infer_and_validate_vdw_fit_info(vdw_term)
            # Contract: vdW affects energies but not vxc/self-consistency.
            self.vdw_eval_mode = "post_density"
        else:
            # Clear any prior vdW-fit contract on refit.
            self.vdw_fit_term = None
            self.vdw_fit_info = None
            self.vdw_eval_mode = None
        if len(noise_nn) == 0:
    ...
    def _compute_mol_covs(
        ...
        for i, mol_id in enumerate(mol_ids):
            ...
            if save_refs:
                ...
                self.vdw_terms_dict[mol_id] = data.get("vdw")
            if deriv:
    ...
    def _compute_mol_covs_parallel(
        self,
        ddir,
        mol_ids,
        kernel_index,
        ...
        if parallel_backend != "process":
            raise ValueError(
                "Only process covariance parallelism is currently supported."
            )
        if chunksize is None or chunksize < 1:
            chunksize = 8
        chunks = list(_chunks(list(mol_ids), chunksize))
        if len(chunks) == 0:
            return
        max_workers = min(int(n_jobs), len(chunks))
        if max_workers <= 1:
            self._compute_mol_covs(
                ddir,
                mol_ids,
                self.kernels[kernel_index],
                get_orb_deriv=get_orb_deriv,
                save_refs=save_refs,
            )
            return
        try:
            ...
            print("Fork multiprocessing is unavailable; falling back to serial.")
            self._compute_mol_covs(
                ddir,
                mol_ids,
                self.kernels[kernel_index],
        ...
        print(
            "Computing molecule covariances in parallel: "
            f"kernel={kernel_index} n_jobs={max_workers} chunks={len(chunks)} "
        ...
        with ProcessPoolExecutor(
            ...
            mp_context=ctx,
            initializer=_parallel_cov_initializer,
            initargs=(self,),
        ) as executor:
            for result in executor.map(_parallel_cov_worker, tasks):
                kernel.cov_dict.update(result["cov"])
                kernel.base_dict.update(result["base"])
                kernel.dcov_dict.update(result["dcov"])
                kernel.dbase_dict.update(result["dbase"])
                self.exx_ref_dict.update(result["exx_ref"])
                self.dexx_ref_dict.update(result["dexx_ref"])
                self.ks_baseline_dict.update(result["ks_baseline"])
                self.vdw_terms_dict.update(result["vdw_terms"])
    ...
    def store_mol_covs(
        ...
        Args:
            ...
            get_correlation (bool): Whether to compute covariance for the
                ...
                (False).
            n_jobs (int): Number of worker processes to use for molecule-level
                covariance construction. The default of 1 preserves serial
                behavior.
            parallel_backend (str): Parallel backend. Currently only "process"
                is supported.
            chunksize (int): Number of molecule IDs per worker task.
        """
        if n_jobs is None:
            n_jobs = 1
        n_jobs = int(n_jobs)
        if _orb_deriv_explicitly_requested(get_orb_deriv) and n_jobs > 1:
            print(
                "Orbital derivative covariance requested; falling back to serial "
                "molecule covariance construction."
            )
            n_jobs = 1
        for i, kernel in enumerate(self.kernels):
            ...
            if get_correlation or kernel.component == "x":
                if n_jobs > 1:
                    self._compute_mol_covs_parallel(
                        ddir,
                        mol_ids,
                        i,
                        get_orb_deriv=deriv,
    ...
    def add_reactions(self, rxn_list):
        ...
        for mode, rxn in rxn_list:
            ...
            self.rxn_mode_list.append(mode)
            self.rxn_structs_list.append(list(rxn["structs"]))
            self.rxn_counts_list.append(list(rxn["counts"]))
